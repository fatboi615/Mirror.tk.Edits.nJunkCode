...
{
	if (GetAsyncKeyState(Menu::Window.RageBotTab.ragetrack.GetKey()))
	{
		...
		for (int i = 0; i < Interfaces::Engine->GetMaxClients(); i++)
		{
			...
			headPositions[i][cmd->command_number % tracking] = backtrackData{ simtime, hitboxpos };
			...
			for (int t = 0; t < 13; ++t)  //going trough all recorded ticks.
			{
				...
				Vector hitboxPos = headPositions[bestTargetIndex][t].hitboxPos;
				headPositions[i][cmd->command_number % tracking] = backtrackData{ simtime, hitboxPos };
				Vector aimpoint = headPositions[bestTargetIndex][t].hitboxPos; //get vector of enemys hitbox
				float FOVDistance = distance_point_to_line(hitboxPos, pLocal->GetEyePosition(), ViewDir); //getting fov to each tick
